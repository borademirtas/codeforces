...
void solve(ll TC, ll TC2) {
    ...
    ll highest = 0;
    ll lowest = 1e10;
    vll a(n); for(ll i = 0; i < n; i++){cin >> a[i]; if(a[i] == -1){ec++;}}
    for(ll i = 1; i < n; i++){
        if(a[i] != -1 && a[i-1] != -1){mx = max(mx, abs(a[i] - a[i-1]));}
        if(a[i] == -1 && a[i-1] != - 1){
            highest = max(highest, a[i-1]);
            lowest = min(lowest, a[i-1]);
        }
    }
    for(ll i = 0; i < n-1; i++){
        if(a[i] == -1 && a[i+1] != - 1){
    ...
    if(ec == n){cout << 0 << " " << 0 << "\n"; return;}
    cout << max(mx, rounddiv(highest-lowest, 2)) << " " <<  (highest + lowest) / 2 <<  "\n";
