...
void solve(ll TC, ll TC2) {
    ...
    for(ll i = 0; i < result.size(); i++){
        if(result[i] == '2'){two = i; break;}
    }
    if(two == -1){cout << n << "\n"; return;}
    ll zero = -1;
    for(ll i = two; i >= 0; i--){
        if(result[i] == '0'){zero = i; break;}
    }
    if(zero == -1){
        ll ans = 1;
        for(ll i = 0; i < result.size(); i++){ans *= 3;}
        cout << ans << "\n"; return;
    }
    result[zero] = '1';
    for(ll i = zero + 1; i < result.size(); i++){result[i] = '0';}
    ll ans = 0; reverse(all(result)); ll mlt = 1;
    for(ll i = 0; i < result.size(); i++){
        ans += (result[i] - '0') * mlt;
        mlt *= 3;
    }
