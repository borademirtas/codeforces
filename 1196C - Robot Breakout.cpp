...
void solve(ll TC, ll TC2) {
    ll n; cin >> n;
    vll x(n); vll y(n); vll x1(n); vll y1(n); vll x2(n); vll y2(n);
    for(ll i = 0; i < n; i++){cin >> x[i] >> y[i] >> x1[i] >> y2[i] >> x2[i] >> y1[i];}
    ll mnx = 1e5 * - 1, mxx = 1e5, mny = 1e5 * -1, mxy = 1e5;
    for(ll i = 0; i < n; i++){
        if(!x1[i]){mnx = max(mnx, x[i]);}
        if(!x2[i]){mxx = min(mxx, x[i]);}
        if(!y1[i]){mny = max(mny, y[i]);}
        if(!y2[i]){mxy = min(mxy, y[i]);}
    }
    if(mnx > mxx || mny > mxy){cout << "0\n"; return;}
    else{
