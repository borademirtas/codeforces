...
void solve(ll TC, ll TC2) {
    ...
    ll n, m; string s; cin >> m >> n >> s; n--; m--;
    ll mnx = 0, mxx = 0, mxy = 0, mny = 0, cx = 0, cy = 0;
    for(ll i = 0; i < s.size(); i++){
        if(s[i] == 'L'){cx--;}  if(s[i] == 'R'){cx++;}  if(s[i] == 'U'){cy++;}  if(s[i] == 'D'){cy--;}
        mnx = min(cx, mnx); mxx =  max(cx, mxx); mny = min(cy, mny); mxy =  max(cy, mxy);
        if(mxx - mnx > n){break;}
        if(mxy - mny > m){break;}
        ans = mp(1 + mxy, abs(mnx) + 1);
    }
