...
ll calc(vll p, ll x, ll n){
    ll i=0, j=n-1;
    ll ans = 0;
    vector<bool> have_gondola_yet(n+1);
    while (i<j){
...
void solve(ll TC, ll TC2) {
    ll n, k; cin >> n >> k; vll a(n); vll cnt(k+1);
    for(ll i = 0; i < n; i++){cin >> a[i]; cnt[a[i]]++;}
    vll pos; for(ll i = 1; i <= k; i++){if(cnt[i] != 0){pos.pb(cnt[i]);} } sort(all(pos));
    ll ans = 1e18; vll nums;
    nums.pb(pos.back());
    ll l = 0, r = pos.size() - 1;
    while(r >= l){
        ll num = 0;
        ...
        if(num > nums[0]){nums.pb(num);}
        l++; r--;
    }
    for(ll i = 0; i < nums.size() ; i++){
        ans = min(ans, calc(pos, nums[i], pos.size()));
    }
    cout << ans;
}
