...
void solve(ll TC, ll TC2) {
    ...
    vll nums{1};
    for(ll i = 1; i < n; i++){
        if(s[i] == s[i-1]){nums[nums.size()-1]++;}
        else{nums.pb(1);}
    }
    ll cnt = 0;
    vll a;
    for(ll i = 0; i < n; i +=2){
        if(s[i]==s[i+1]){a.pb(s[i] - '0');}
        else{a.pb(-1);}
    }
    for(ll i = 1; i < a.size(); i++){if(a[i] == -1 && a[i-1] != -1){a[i] = a[i-1];}} reverse(all(a));
    for(ll i = 1; i < a.size(); i++){if(a[i] == -1 && a[i-1] != -1){a[i] = a[i-1];}}
    ll ans = 1;
    for(ll i = 1; i < a.size(); i++) {if(a[i] != a[i-1]){ans++;}}
    for(ll i = 1; i < nums.size(); i++){
        if(nums[i-1]&1){cnt++; nums[i-1]--; nums[i]++;}
    }
