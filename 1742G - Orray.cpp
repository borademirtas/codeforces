...
void solve(ll TC, ll TC2) {
    ll n;
    cin >> n; vll a(n); for(ll i = 0; i < n; i++){cin >> a[i];} sort(all(a)); ll num = a.back(); cout << num << " ";
    vector<bool> used(n); used[n-1] = true;
    for(ll i = 0; i < min(n-1, 30LL); i++){
        ll best = -1, bp = -1;
        for(ll j = 0; j < n; j++){
            if(used[j]){continue;}
            if((num|a[j]) > best){best = num | a[j]; bp = j; }
        }
        cout << a[bp] << " "; num = best; used[bp] = true;
    }
    for(ll i = 0; i < n; i++){
        if(!used[i]){cout << a[i] << " ";}
    }
