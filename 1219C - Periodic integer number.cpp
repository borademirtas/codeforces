...
void solve(ll TC, ll TC2) {
    ll n; string s;
    cin >> n >> s; ll sz = s.size();
    bool a9 = true;
    for(ll i = 0; i < sz; i++){if(s[i] != '9'){a9 = false;}}
    if(a9){sz++;}
    if(sz % n != 0 || a9 || n > sz){
        ...
        string temp;
        for(ll i = 0; i < n; i++){temp.pb(s[i]);}
        string nt;
        while(nt.size() < sz){nt += temp;}
        if(nt > s){cout << nt; return;}
        nt.clear();
        reverse(all(temp));
        ll carry = 1;
        for(ll i = 0; i < n; i++){
