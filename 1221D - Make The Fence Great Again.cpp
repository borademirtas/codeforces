...
void solve(ll TC, ll TC2) {
    ll n; cin >> n;  vector<vector<ll>> dp(10, vector<ll>(n, 1e18+1)); vll a(n);
    for(ll i = 0; i < n; i++){
        cin >> a[i]; ll x; cin >> x;
        if(i == 0){
            for(ll j = 0; j < 10; j++){dp[j][0] =  x * j;}
        }
        else{
            for(ll j = 0; j < 10; j++){
                for(ll t = 0; t < 10; t++){
                    if(a[i-1]+j != a[i] + t && dp[j][i-1] != 1e18+1){
                        dp[t][i] = min(dp[j][i-1] + x * t, dp[t][i]);
                    }
                }
            }
        }
