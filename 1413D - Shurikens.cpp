...
void solve(ll TC, ll TC2) {
    ll n; cin >> n;
    set<ll> s; vll ans(n*2); vll recent;
    vector<pair<char, ll>> a(n*2);
    for(ll i = 0; i < n * 2; i++){
        cin >> a[i].first;
        if(a[i].first == '+'){
            recent.pb(i);
            ...
            cin >> a[i].second;
            if(recent.size() == 0){cout << "NO\n"; return;}
            ll x = recent.back(); recent.pop_back();
            ans[x] = a[i].second;
        }
    }
    for(ll i = 0; i < ans.size(); i++){
        if(a[i].first == '+'){
            ...
            if(*s.begin() < a[i].second){cout << "NO\n"; return;}
            else{s.erase(a[i].second);}
        }
    }
    cout << "YES\n";
    for(ll i = 0; i < ans.size(); i++){
