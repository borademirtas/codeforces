...
string shift(string x){
    vector<char> ret(x.size());
    for(ll i = 0; i < x.size(); i++){
        ret[(i+1) % x.size()] = x[i];
    }
    for(ll i = 0; i < x.size(); i++){x[i] = ret[i]; }
    return x;
}

void solve(ll TC, ll TC2) {
    cin >> n >> s; a.resize(n); ll ans = 1;
    vll used(n+1); for(ll i = 0; i < n; i++){cin >> a[i]; a[i]--;}
    for(ll i = 0; i < n; i++){
        if(used[i]){continue;}
        string x; x.pb(s[i]);
        ll np = a[i]; used[i] = 1;
        ...
        ll cnt = 1; string s1 = x; s1 = shift(s1);
        while(true){
            if(s1 == x){break;}
            else{cnt++; s1 = shift(s1);}
        }
        ans = lcm(ans, cnt);
    }
    cout << ans << "\n";
