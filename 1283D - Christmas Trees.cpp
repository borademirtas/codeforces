...
void solve(ll TC, ll TC2) {
    ll n, m; cin >> n >> m;  mpll used;  vll ans; ll sum = 0, cnt = 0; queue<pair<ll,ll>> q;
    vll a(n); for(ll i = 0; i < n; i++){cin >> a[i]; used[a[i]]++; q.push(mp(1, a[i]+1));  q.push(mp(1, a[i]-1)); }
    while(cnt < m){
        ...
        if(used[cur.second]){continue;}
            ...
            ans.pb(cur.second);
            cnt++;
            q.push(mp(cur.first + 1, cur.second+1));  q.push(mp(cur.first + 1, cur.second-1));
            used[cur.second]++;
        }
    }
    cout << sum << "\n";
