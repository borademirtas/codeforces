...
void solve(ll TC, ll TC2) {
    ll n; cin >> n; vll a(n); mpll b; vll u; mpll pos; ll ct = 0; for(ll i = 0; i < n; i++){cin >> a[i];b[a[i]]++; if(b[a[i]] == 1){u.pb(a[i]);}}
    ll hs = 0;
    ...
    ll flag = -1;
    if(ct == 1 && hs == 0){cout << "NO\n"; return;}
    if(ct == 1){
        for(ll i = 0; i < u.size(); i++){
            if(b[u[i]] > 2){flag = u[i]; break;}
        }
    }
    ct = 0;
    for(ll i = 0; i < u.size(); i++){
    ...
    for(ll i = 0 ; i < n; i++){
        cnt[a[i]]++;
        if(b[a[i]] ==  2 || a[i] == flag ){ans[i] = 'A';}
        else{
            if(cnt[a[i]] == 1){
                if(pos[a[i]] == 0){ans[i] = 'A';}
                else{ans[i] = 'B';}
            }
            else{
                if(pos[a[i]] == 0){ans[i] = 'B';}
                else{ans[i] = 'A';}
            }
        }
    }
