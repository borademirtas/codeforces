...
void fill(ll n, ll num, ll st){
    for(ll i = st; i <= n; i++){ans[i] = num;}
}
...
void solve(ll TC, ll TC2) {
    ...
    for(ll i = 0; i < n; i++){
        if(ans[a[i]] != -1){cout << ans[a[i]] << " "; continue;}
        ll p = a[i];
        for(ll j = a[i] - 1; j >= a[i] - k; j--){
            p = j;
            if(j == -1 || ans[j] != -1){ break;}
        }
        if(p == -1 || a[i] - ans[p] + 1 > k){fill(a[i], p + 1, p + 1);}
        else{fill(a[i], ans[p], p + 1);}
        cout << ans[a[i]] << " ";
    }
}
