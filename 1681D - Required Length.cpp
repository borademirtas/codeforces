...
void solve(ll TC, ll TC2) {
    mpll used;
    ll n, x; cin >> n >> x;
    string s = to_string(x); sort(all(s));  if(s.size() >= n){cout << 0; return;} if(s.back() == '1'){cout << -1; return;}
    queue<pair<ll,ll>> q; q.push(mp(x,0));
    while(!q.empty()){
        ll num = q.front().first; s = to_string(num); ll pos = q.front().second; q.pop();
        if(s.size() >= n){cout << pos; return;}
        if(used[num] == 0){
            for(ll i = 0; i < s.size(); i++){
                q.push(mp((s[i] - '0') * num, pos+1));
            }
            used[num]++;
        }
    }
