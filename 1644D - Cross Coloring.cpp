...
void solve(ll TC, ll TC2) {
    ...
    vp a(q);  for(ll i = 0; i < q ;i++){cin >> a[i].first >> a[i].second;}
    mpll cntx; mpll cnty;
    set<ll> x; set<ll> y;
    ll ans = 0;
    for(ll i = q -1 ; i >= 0; i--){
        bool ok = true;
        if(cntx[a[i].first] > 0 && cnty[a[i].second] > 0){ok =  false;}
        cntx[a[i].first]++; cnty[a[i].second]++;
        if(y.size() == m){ok = false;}
        if(x.size() == n){ok = false;}
        y.insert(a[i].second);  x.insert(a[i].first);
        if(ok){ans++;}
    }
    ll num = binpow(k, ans, 998244353);
    cout << num << "\n";
}
