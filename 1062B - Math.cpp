...
void solve(ll TC, ll TC2) {
    ...
    vll a = prime(n); mpll b;  vll u;
    for(ll i = 0; i < a.size(); i++){b[a[i]]++; if(b[a[i]] == 1){u.pb(a[i]);}}
    ll mx = 0;
    ll ans = 1;
    ...
    if(mx == 1){cout << ans << " " << 0; return; }
    ll add = 1; for(ll i = 2; i <= mx * 2; i *= 2){
        if(mx <= i){break;}
        else{add++;}
    }
    ll num = pow(2, add); bool check = true; for(ll i = 0; i < u.size(); i++){if(b[u[i]] != num){check = false;} }
    if(!check){add++;;}
    cout << ans << " " << add;
}
