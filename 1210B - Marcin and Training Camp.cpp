...
void solve(ll TC, ll TC2) {
    ...
    vll u;
    ll ans = 0;
    for(ll i = 0; i < n; i++){cin >> a[i].first; b[a[i].first]++; if(b[a[i].first] == 2){u.pb(a[i].first); k[a[i].first]++;} } for(ll i = 0; i < n; i++){cin >> a[i].second;}
    for(ll i = 0; i < n; i++){
        if(k[a[i].first] == 1){ans += a[i].second; continue;}
        bool ok = false;
        const ll num = a[i].first;
        string b1 = std::bitset<60>(num).to_string();
        for(ll j = 0; j < u.size(); j++){
            const ll n2 = u[j];
            string b2 = std::bitset<60>(n2).to_string();
            bool ok2 = true;
            for(ll t = 0; t < b2.size(); t++){
                if(b2[t] == '0' && b1[t] == '1'){ok2 = false; break;}
            }
