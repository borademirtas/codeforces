...
void solve(ll TC, ll TC2) {
    ll n, ac = 0, bc = 0; string s; string t;
    cin >> n >>  s >> t;
    vll ba;
    vll ab;
    vp ans;
    for(ll i = 0; i < n; i++){
        ac += (s[i] == 'a');  bc += (s[i] == 'b');  ac += (t[i] == 'a');  bc += (t[i] == 'b');
        if(s[i] == 'a' && t[i] == 'b'){ab.pb(i);}  if(s[i] == 'b' && t[i] == 'a'){ba.pb(i);}
    }
    if(ac % 2 == 1 || bc % 2 == 1){cout << -1; return;}
    while(!ba.empty() || !ab.empty()){
        if(ba.size() >= 2){
            ans.pb(mp(ba[ba.size()-1], ba[ba.size() -2]));
            ba.pop_back(); ba.pop_back();
            continue;
        }
        else if(ab.size() >= 2){
            ans.pb(mp(ab[ab.size()-1], ab[ab.size() -2]));
            ab.pop_back(); ab.pop_back();
            continue;
        }
        else{
            ans.pb(mp(ab[ab.size()-1], ab[ab.size() -1]));
            ba.pb(ab[ab.size()-1]); ab.pop_back();
            continue;
        }
