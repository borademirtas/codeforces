...
void solve(ll TC, ll TC2) {
    ll n; cin >> n;
    vll nums; mpll cnt; vp lines(n);
    for(ll i = 0; i < n; i++){
        cin >> lines[i].first >> lines[i].second; cnt[lines[i].first]++; if(cnt[lines[i].first] == 1){nums.pb(lines[i].first);}
        cnt[lines[i].second]++; if(cnt[lines[i].second] == 1){nums.pb(lines[i].second);}
    }
    sort(all(nums)); mpll pos; ll ans = n - 1;
    for(ll i = 0; i < nums.size(); i++){pos[nums[i]] = i + 1;}
    vll front(nums.size()+2);
    vll back(nums.size()+2);
    for(ll i = 0; i < n; i++){
        lines[i].first = pos[lines[i].first];
        lines[i].second = pos[lines[i].second];
        front[lines[i].second]++;
        back[lines[i].first]++;
    }
    for(ll i = 1; i <= nums.size(); i++){front[i] += front[i-1];}
    for(ll i = nums.size() - 1; i >= 0; i--){back[i] += back[i+1];}
    for(ll i = 0; i < n; i++){
        ans = min(ans, front[lines[i].first-1] + back[lines[i].second+1]);
    }
    cout << ans << "\n";
}
