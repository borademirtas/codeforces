...
void solve(ll TC, ll TC2) {
    ll n; cin >> n; n *= 2; multiset<ll> s;
    vll a(n); for(ll i = 0; i < n; i++){ll x; cin >> x; s.insert(x);}
    ll num = 2750131;
    vector<bool> is_prime(num+1, true);
    ll pc = 1; vll pcnt(num+1);
    ...
    for(ll i = 2; i <= num; i++){
        if(is_prime[i]){pcnt[i] = pc; pc++;}
    }
    while(!s.empty()){
        ll x = *--s.end(); s.erase(s.lower_bound(x));
        if(is_prime[x]){
            ll y = pcnt[x]; cout << y << " "; s.erase(s.lower_bound(y));
        }
        else{
            ll y = smallestDivisor(x); cout << x << " "; s.erase(s.lower_bound(y));
        }
    }
